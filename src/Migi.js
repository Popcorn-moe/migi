...
export default class Migi extends Client {
	...
	loadConfig(name, defaultConfig) {
		...
		if (existsSync(path))
			...
			const config = json5.stringify(defaultConfig, {
				space: '\t'
			})
				.split(`\n`)
				.map((line, i, { length }) => {
	...
	_onMessage(message) {
		...
		for (const [module, { commands }] of this._modules.entries()) {
			for (const [regex, key, { prefix = true }] of commands) {
				if (prefix && !content.startsWith(this.settings.prefix))
					continue
				const result = regex.exec(prefix ? content.slice(this.settings.prefix.length) : content)

				if (result)
					this._call(module, key, message, ...result.slice(1))
